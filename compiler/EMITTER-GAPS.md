...
- **#23 / slice-7 SIGTRAP — constrained HOF whose own Num constraint comes from internal literals (`app2 f = f 2 3; main = println (app2 (==))`)** 🔴 **OPEN — verified diagnosis, fix DEFERRED (2026-06-28).** A user HOF generalized to `(Num a, Num b) => …` from literals applied INSIDE its body (`app2 f = f 2 3`, `useIt h = h 2 3`), called with a function that ties those Num vars to ANOTHER class and yields a concrete result — `app2 (==)` / `useIt eqBoth` (the var is `Num`+`Eq`, the result `Bool`) — BUILDS but the binary exits 133 (SIGTRAP), while `medaka run` prints the correct value. **run≠build silent miscompile.** **Verified root cause (NOT the GAP3-SLICE7-DESIGN.md `debug`/`sequence` arg-stamp framing — the manifestation shifted post-#21):** the HOF's Num constraint var is grounded by NEITHER an argument NOR the caller's result type (`main : Unit`), so it is genuinely ambiguous and must DEFAULT to Int (PLAN #11). It does not: the callee's instantiated Num constraints are recorded on `pendingCallObligations` (and, for an INFERRED-constraint promoted fn like `app2`, captured in `pendingDictApps` via `inferDictAtFound`), but the Num-defaulting pass (`processSCC`→`defaultGroupNum`/`defaultEachMember`, `typecheck.mdk`) reads ONLY this binding's `pendingImplObligations` delta — so a literal lexically in the caller (`eqBoth 2 3`, which WORKS) grounds, but a literal hidden inside the called HOF (`useIt eqBoth`, which SIGTRAPs) does not. Ungrounded ⇒ the callee's `CDict` Num-dict routes resolve `RNone` (`routeOfMonoTop`) ⇒ a NULL (`0`) dict word at the call ⇒ `inttoptr 0; load` in the callee body ⇒ SIGTRAP. **Two targeted fix attempts FAILED** (a clean STOP, per scope discipline): (1) relaxing `defaultAmbiguousNum`'s `containsI id inT` filter — irrelevant, `main` defaults via `defaultGroupNum` which already drops that filter; (2) folding this SCC's new `pendingCallObligations` Num delta into the defaulting candidate set — ineffective, because `app2`'s INFERRED Num constraint flows through the `pendingDictApps` (promotion) path, not `pendingCallObligations`. **Next step:** ground the ambiguous Num monos captured in the per-SCC `pendingDictApps` delta (or unify the inferred-constraint promotion obligation flow with `pendingCallObligations`) so defaulting sees them — verify with `app2 (==)` / `useIt eqBoth` run==build, full `diff_compiler_*` + fixpoint (the highest-traffic defaulting path; gate hard). Niche shape (a real silent miscompile but no current stdlib caller); the clean compile-error face (E24) is the common one and is now closed.

...
1. **The observable D2 collision is fully closed for every shape asked for.**
   ...
   - the loader-driven typed-modules gate (`diff_compiler_eval_typed_modules`,
...
2. **Claim 2 from the second scoping pass is REFUTED — the bare
   ...
   `compiler/types/typecheck.mdk:8136`
   ...
   (`compiler/types/typecheck.mdk:2642`) carries **no constraint list**. The set
...
- **`annotateProgram` / `EVarAt` is NOT a live precedent to mirror.** It lives in
  `compiler/types/annotate.mdk` (not `resolve.mdk`), is **exported but unwired**
  (drivers never run it; eval's `EVarAt` arm is dormant — see `compiler/README.md`
...
1. **AST** (`compiler/frontend/ast.mdk` ~`:184`, beside `EVarAt`): add a
...
2. **Resolve** (`compiler/frontend/resolve.mdk`): a new post-resolve rewrite that
...
3. **Printer/fmt** (`compiler/tools/*printer*`): render `EVarFrom n _` as bare `n`
   (like `EVarAt`/`EDictApp`) or `diff_compiler_fmt` + round-trip break.
4. **Typecheck re-key** (`compiler/types/typecheck.mdk`): re-key the six refs to
...
- No `compiler/` code change (the re-key is deferred).

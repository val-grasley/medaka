...
- **Native-only** throughout (the OCaml oracle is frozen + slated for removal;
  ...
  `diff_compiler_check_json` (native vs LIVE oracle json, byte-identical) — adding
...
- **Human messages already exist** for every class: `ppResError`
  (`compiler/frontend/resolve.mdk:937`) + typecheck `(msg, Option Loc)` pairs. The
...
CLI forks at `compiler/driver/medaka_cli.mdk:124`:
- **plain text** → `runCheckCmd` → `checkRoute` → on error: `ppDiagCliSrc` (carat);
  on success: `runCheck` (`compiler/tools/check.mdk:49`) — full scheme dump
  (gated byte-identical — do NOT disturb).
- **`--json`** → `runCheckJsonCmd` (`medaka_cli.mdk:293`) → `analyzeLocated`
  (`compiler/driver/diagnostics.mdk:146`) — humane + positioned for all error types;
...
- **Re-route the plain ERROR path only** through located diagnostics
  ...
  `runCheck` would disturb `diff_compiler_check`'s clean-fixture goldens).
...
- **Gate:** add parse + type fixtures to `test/check_json_fixtures/` (positions vs
  ...
  `diff_compiler_check` goldens for the new plain text.
...
- `compiler/frontend/resolve.mdk` — `ResError` type :65, `resErrorSexp` :885,
  `ppResError` :937 (all ctors now carry `Option Loc` — Stage B complete).
- `compiler/driver/diagnostics.mdk` — `ppDiag` :71, `ppDiagCli`/`ppDiagCliSrc` :89-105,
  `analyzeFrom` :153 (resolve `loc=None` pattern match at :104; `resErrorLoc e` used at :160).
- `compiler/driver/medaka_cli.mdk` — `runCheckCmd` :104, `runCheckJsonCmd` :293,
  parse-err block :130-134 (inside `runCheckCmd`).
- `compiler/tools/check.mdk` — `runCheck` :49, `routeImportCheck` :67 (message
  swap; host of `diff_compiler_check`).
- `test/diff_compiler_check_json.sh` + `test/check_json_fixtures/` — the only

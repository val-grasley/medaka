...
**Status:** IMPLEMENTED — essentially everything enumerated is marked CLOSED with a date
in the doc's own body, several re-verified well after initial closure. Implements
`compiler/DRIVER-COLLAPSE-PLAN.md` §6 (independently confirmed IMPLEMENTED). One item
remains explicitly open: the `empty`/`foldMap` default (`empty@?` RNone site, see
`DISPATCH-INVENTORY.md`). Dead bare `compiler/*.mdk` path citations (~19) corrected to
their post-restructure subfolder paths, 2026-07-13 doc pass. The "OCaml reference (`lib/`)
as oracle" framing throughout is historical (accurate for the 2026-06-09 audit date,
predates the 2026-06-26 OCaml removal).

Audit of `compiler/types/typecheck.mdk` (HM core + dict-passing layer), `compiler/frontend/marker.mdk`,
and the typecheck↔eval seam (`compiler/eval/eval.mdk`, drivers, `compiler/types/annotate.mdk`),
...
**FIXED:** ported the awaits-args gate into `compiler/eval/eval.mdk` `EMethodAt` (758-771,
...
- **Where:** `compiler/eval/eval.mdk:758-762`; oracle gate `lib/eval.ml:869-873`.
- **What:** the oracle folds method-/impl-dicts into the narrowed value only when it
  ...
  *terminal* impl body (gets no leading dict param, by `usesImplDict` design,
  `compiler/types/typecheck.mdk:2606-2656`) receives a dict argument anyway.
...
- **Where:** `compiler/types/typecheck.mdk:2595-2614` (`dictPassDecl` DImpl arm →
...
- **Done (2026-06-09):** ported `check_coherence` into `compiler/types/typecheck.mdk` as
...
- **Where:** all five generalize sites generalize unconditionally:
  `compiler/types/typecheck.mdk:1606` (inferLetBody), `:1390-1395` (blockLet), `:1844-1847`
...
- **Adjacent oracle hole = T1b — ✅ CLOSED (2026-06-09, both sides):**
  ...
  Selfhost: `blockLet` (`compiler/types/typecheck.mdk:1451-1456`) now takes the `mut`
...
- **Fix landed (2026-06-09):** ported `isNonexpansive` / `lowerToCurrent` /
  `genRestricted` into `compiler/types/typecheck.mdk` (after `generalize`) and threaded
...
- **Where:** `compiler/types/typecheck.mdk:1204` (`infer env (ELet _ _ pat e1 e2)` — both
...
- **Done (2026-06-09):** split `inferLet` into three arms (`compiler/types/typecheck.mdk`):
...
- **Where (original):** `compiler/types/typecheck.mdk` (`moduleDictNames` global bare-name
...
- **Verdict: MOOTED by universal per-module mangling (`332ef41`).** `mangleUnits`
  (`compiler/backend/private_mangle.mdk`) runs in `runEmit`
...
- **Where:** `compiler/types/typecheck.mdk` `routeOfMono` (concrete arm returned
...
- **Done (2026-06-09):** rewrote `inferFieldAccess` / `inferRecordUpdateField` in
  `compiler/types/typecheck.mdk` to mirror lib/typecheck.ml's `EFieldAccess` (Phase 72). New
...
**Fix:** added `checkMissingFields` (`compiler/types/typecheck.mdk:1547-1559`) — iterates the

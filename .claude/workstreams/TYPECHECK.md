...
| Operator seams ×4 | LANDED #146 → collapsed to `recordIfaceObligation`/`ifaceRegistered`/`ifaceEntryMatches` (the 12 clones `record{Num,Eq,Ord,Semigroup}Obligation` + `*IfaceRegistered` guards + `*Entry` predicates are retired; O(1) rep of `methodIfaceParamsRef` is the follow-up #147) |

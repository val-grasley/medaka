...
- **Next clean SQLite bites (from the scout):** ORDER BY in the query ADT (small, pure-lib, `select.mdk`); `UPDATE`/`DELETE` (mutation — larger); WasmGC port of the bytes-first API; async SQL server.

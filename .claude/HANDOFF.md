...
- **Design doc:** `compiler/FMT-COMMENT-INTERLEAVING-DESIGN.md` (`75251e8`). **Deferred polish (non-blocking, tasks open):** T — O/P/R residual, a single-application body ~80 chars at indent still spine-breaks instead of hanging at `=`; L Stages 3-5 — full Doc-IR comment interleaving (comments travel WITH their operand through reflow, vs current verbatim-preserve) per the design doc, scope chains+block/do/let. Memory: `project_fmt_hardening_and_commit_hook`.

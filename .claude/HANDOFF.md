...
- **#24 — wasm real-prelude point-free impl arity** (pre-existing, low pri): foldMap-class impls emit an UNDER-APPLIED `fold step empty` (missing the container arg) → `assemble_check_main` validate fails (func 1800) + `diff_wasm_modules` fully GAP; blocks full wasm validation of real-prelude programs. Wasm-only.

...
## RESUME — 🟡 `medaka lint` pre-commit hook LIVE + linter hardened; "plain lint → 0" cleanup IN FLIGHT (2026-07-02). `main` = `62fe6ea`; 4 unverified Wave-A branches to land
Extended the pre-commit hook to **also gate `medaka lint`** (mirroring the fmt hook). **CORE DELIVERABLE DONE + MERGED to `main`** (through commit `e9ab626`); the "drive *plain* `medaka lint` to 0 warnings" cleanup is **mid-flight** (Wave A salvaged to branches, unverified; Waves B/C not started). Owning memory: `project_lint_precommit_hook`.
...
### 🟡 IN FLIGHT — user goal: **plain `medaka lint` = 0** (so a new warning is unambiguously new, not pre-existing). NOT project-specific rule refinements — user was explicit: **linter rules must stay general; use inline/file DIRECTIVES for project-specific suppressions.**
**Grounded triage (2 read-only agents, do NOT re-triage — trust these):**
- **`rule-match-on-param` (127): 123 FIX / 4 SUPPRESS.** Both emitters 100% convertible (idiomatic multi-clause AST dispatch, `f a (CFoo x)=…`). Only SUPPRESS: the 4 `Thenable m =>` `traverse` impls (`core.mdk` ×3 @808/814/821, `validation.mdk` @73) — multi-clause + return-position `pure` LOOPS in eval (documented core.mdk ~804). `--fix --only=rule-match-on-param` does the comment-free auto ones; comment/as-pattern/scrutinee-reuse ones need manual (as-pattern `r@(Bin …)`).
- **`rule-duplicate-body` (232): ~119 FIX / ~113 SUPPRESS.** FIX = extract shared modules: entry-probe scaffolding→new `entry_support.mdk` (~70), `cj*`/`readDiagSrc`→`diagnostics.mdk` (18), `eagerVars`+backend-neutral emit helpers (`topo*`/`methodArityOf`/`hashName`/`safeChar`)→`core_ir.mdk`/`private_mangle.mdk`/new `emit_support.mdk` (~29), sqlite lib `nthCell`→`recordfmt.mdk` (2). SUPPRESS (justified, use directives): distinct-ADT map/set + hash_map/hash_set balancing/rehash helpers (need `Set=Map _ Unit` refactor — out of scope), hex/base64 3-line codec wrappers, wasm-only `*W` extern classifiers (expected to diverge), self-contained sqlite demos (EXCEPT the ~13 type-independent printers `printRows`/`printInts`/… → extract to `sqlite/demo_support.mdk`). User chose: extract type-independent demo printers only; leave type-coupled demo helpers standalone + suppress.

**Wave A — SALVAGED to branches, UNVERIFIED (agents killed at session limit mid-verification). Base = `e9ab626` (behind current `main`=62fe6ea).** For EACH: `git merge main`, rebuild `./medaka`, run gates, THEN merge. Disjoint files so they can merge in any order — EXCEPT A4 conflicts (see below).
- **A1 `df9dc2f`** (worktree `agent-af9a4f7e4d44bb4e2`): new `compiler/entries/entry_support.mdk` + refactored ~6 probe entries (eval_typed/dict, core_ir_typed/dict_pp, llvm/wasm_emit_typed). Entries are `medaka run` probes — NOT in seed, no re-mint; gated by `diff_compiler_*` probe suites (must stay byte-identical). Did NOT touch playground_main/medaka_cli (A2's). Report showed 7 files; may not have finished all in-scope probes (batch family) — check residual `lint compiler | grep duplicate-body`.
- **A2 `2c10650`** (worktree `agent-a6226b7da66d7726f`): `cj*`+`readDiagSrc` moved into `diagnostics.mdk`; deleted from `medaka_cli.mdk` + `playground_main.mdk`. Verify: `diff_native_cli` (stale-prone → `FORCE=1 build_oracles` first), playground build, fixpoint.
- **A3 `e670a7f`** (worktree `agent-a042c302b57621b76`): sqlite — `nthCell`→`recordfmt`, new `sqlite/demo_support.mdk` (was left UNTRACKED, I `--amend`ed it in), match-on-param fixes in `lib/dbwriter.mdk`+`lib/mutate.mdk`. Verify: sqlite `*_oracle.sh` for touched files. (Stray untracked build binary `sqlite/update_expr_demo` left in worktree — do NOT commit; belongs in .gitignore.)
- **A4 `28d7026`** (worktree `agent-a01623429ee039048`): stdlib match-on-param — `fromList`/`orElse`/`rotateL`/`rotateR` → multi-clause + 4 `traverse` inline-suppressed. ⚠️ **WILL CONFLICT on `stdlib/core.mdk`** — the concurrent session's `62fe6ea`/`dbb155b` added FP combinators to core.mdk. Resolve the merge, and re-check core.mdk for NEW match-on-param from their additions. Verify: `medaka test stdlib/core.mdk`, `diff_compiler_eval_run`/`check`, and a `traverse [Some 1, Some 2]` run (a bad multi-clause would HANG).

### ⏭️ REMAINING (not started):
- **Wave B (emitter, ONE seed re-mint):** convert all match-on-param in `llvm_emit.mdk`(14)+`wasm_emit.mdk`(97) to multi-clause (100% convertible; `--fix` is comment-safe but bails on ~23 comment/as-pattern decls → manual those), AND duplicate-body FIX-3/4 (`eagerVars`→core_ir, mangling/topo/dispatch helpers→private_mangle/new emit_support). Batch emitter edits → verify `selfcompile_fixpoint` C3a/C3b each; orchestrator re-mints seed ONCE at the end (`test/refresh_seed.sh` then `bootstrap_from_seed`).
- **Wave C (reach 0):** add inline/file `-- lint-disable-file rule-duplicate-body` directives to the SUPPRESS files (distinct-ADT stdlib pairs, hex/base64, wasm `*W`, residual sqlite demos) + confirm A4's 4 match-on-param suppressions. Then assert `medaka lint compiler|stdlib|sqlite` = **0** for both advisory rules. Reconcile AGENTS.md + memory.
- **Consider (general, non-project-specific) rule question:** whether `rule-match-on-param` should structurally not-flag impl methods where multi-clause is unsafe — deferred; using directives for now per user's "keep rules general."

**Process notes:** 3 of 4 Wave-A agents hit the empty-report "waiting for oracle build" mode (salvaged per playbook — committed WIP, reaped orphan build_oracles, A3's new file was untracked under `-a` → amended). All Wave A UNVERIFIED — next session MUST build+gate each before merging. Concurrent stdlib session moved `main` throughout (FP-combinator work in core.mdk); merges were clean 3-way except A4/core.mdk.

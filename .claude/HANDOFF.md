...
## RESUME — ✅ Compiler MAY use stdlib; OrdMap→stdlib `Map` shipped + policy changed (2026-06-29). Commit `59f0545` (worktree branch `worktree-snazzy-rolling-alpaca`, not yet merged to `main`)
...
- **Next:** merge to `main` if approved; then `support/util.mdk` → stdlib `list`/`string` as a second *measured* step (`HELPER-CENSUS.md`). Open language idea: make the typechecker expand type aliases (would let `type OrdMap a = Map String a` drop the wrapper).

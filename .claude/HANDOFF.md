...
- **🚧 RESIDUAL — L Stage 5 (block/do/let statement-level interior comments): INCOMPLETE, WIP preserved on branch `wip/L-stage5-blocks` (`4d947ca`).** Session-limit-killed mid-implementation. It adds a **parser.mdk side-channel** (operand→source-line) — parser IS in the emitter graph, so completing it likely OWES A SEED RE-MINT (verify via fixpoint C3a). To finish: check out the branch, complete + gate + fixpoint-check + re-mint if C3a fails. **Not blocking** — the verbatim safety-net already makes block interior-comments CORRECT (just not reformatted); chains-only is a fine stopping point.

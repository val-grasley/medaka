...
| N6 — total-order story | ✗ undecided | owner decision **#360**; interim bar (engine uniformity) **met** on `compare`/`min`/`max` (eval==native==wasm: `Eq`, `nan`, `nan`, `1.0`, `1.0` — probe), **violated** on HOF-routed `<=`/`>=` (eval `False`, native+wasm `True` — #305) |
| N7 — conversions total | ✗ CONFIRMED | `floatToInt` = unguarded `i64.trunc_f64_s`: NaN/inf/range **traps uncoded** where eval+native return `0` — **#372** (blocked on #346's owner pin); `intToFloat` ✅ (`f64.convert_i64_s`) |

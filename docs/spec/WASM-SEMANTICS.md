...
| N6 — total-order story | ✗ decided, unimplemented | owner DECIDED 2026-07-16 (**#360**): `compare`/`min`/`max`/`sort` at NaN = IEEE-754 totalOrder; derived `<`/`<=`/`>`/`>=` stay primitive IEEE on every path (the principled #305 fix). Current state: interim uniformity **met** on `compare`/`min`/`max` (eval==native==wasm: `Eq`, `nan`, `nan`, `1.0`, `1.0` — probe, and re-verified UNPERTURBED by the #305 fix) and now **met** on HOF-routed `<`/`<=`/`>`/`>=` too (eval==native==wasm `False` — **#305 FIXED**). totalOrder for `compare`/`min`/`max`/`sort` remains unimplemented (#360) |

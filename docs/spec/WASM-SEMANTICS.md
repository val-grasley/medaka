...
| R5 — no UB | ⚠ | the `INT64_MIN /(−1)` div-overflow trap is **GONE** — #368's renormalization makes i64 `INT64_MIN` unreachable, so the claim it invalidated holds again (`arith_int63_div_intmin`); residual: dispatch-chain `unreachable`s cite coherence but are invalidated under #377 collision / #324 key-space mismatch, and #372's `i64.trunc_f64_s` still traps uncoded |

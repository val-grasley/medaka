...
| `llvm_emit_typed_main.mdk` | TYPED entry for the LLVM spike (slices 6–7 — dispatch needs types; analog of `core_ir_typed_main.mdk`): `medaka run selfhost/llvm_emit_typed_main.mdk <runtime.mdk> <prelude.mdk>... <src.mdk>` desugars → `typecheck.elaborateDict` (stamps EMethodAt/EDictAt routes + `dict_pass` leading dict params) → lowers (routes read out into `CMethod`/`CDict`; arg-dispatched methods stay a bare `CVar`) → emits LLVM. The ONLY LLVM driver that produces dispatch (the plain `llvm_emit_main` path never marks). `test/diff_selfhost_llvm_typed.sh` gates it over `test/llvm_fixtures_typed/` (6 prelude-free dispatch fixtures: slice 6 — single-impl `RKey`, multi-impl `RKey` narrowed at two types, a `=>`-constrained fn through the dict at two types; slice 7 — single-impl arg-dispatched multi-clause, multi-impl at distinct ADTs, a multi-clause wildcard fall-through). Oracle = the TYPED Core-IR tree-walker `core_ir_dict_pp_main.mdk` (the SAME lowered IR ceval'd) — **not** `eval_probe`, which is untyped and leaks the dispatch wrapper (`<impl@Int:7>`); fixtures pass only `runtime.mdk`, so `elaborateDict` resolves every route without pulling `core.mdk` into the scalar module. 6/6 byte-identical. |

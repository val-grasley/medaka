...
- **`max`/`min` over PRIMITIVE `Ord` receivers** — **SPLIT (2026-06-11).** Two distinct mechanisms hid under "the 2 max/min census events":
  ...
  - ⏳ **arg-tag RNone over primitives still OPEN** — `maximum`/`minimum` in core.mdk call `max`/`min` with route **RNone** (no dict, no concrete tag): the only `compare` impls are at primitive types (`Int`/`String`/…) which carry no runtime cell tag, so `emitMethodArgDispatch`→`emitDefaultArgTag` gaps / SIGSEGVs. `maximum [3,7]` still aborts (exit 139). This is the facet that BLOCKS the real-prelude flip when those helpers are reachable; fix path unchanged (RNone→primitive-typed default-dispatch, or DCE the unreachable bodies). Deferred.

...
- **`CTGuard` — clause/match GUARDS are not lowered.** `emitTree`'s `CTGuard` arm is a gap: under the gap-tolerant self-compile build a guarded arm lowers to a `store i64 0` placeholder, so the function **silently returns 0/Unit**. **Worked around by the GUARD-FREE convention**: all seven stage modules + `llvm_emit.mdk` are written guard-free (use `if`/`else` or the `function` form, never `| guard =`). Surfaced at self-compile step C1 — `llvm_emit.mdk` was the last module still carrying guarded arms (`emitGroupBody`/`emitCmp`), which blanked their bodies natively; rewritten guard-free (no semantics change). **Documented as an AGENTS.md gotcha.** CLOSING IT (emit a real guard test + branch in the `CTGuard` arm, threading the next-clause fallthrough label the way `emitClauseChain` already does for whole-clause fall-through) would retire the convention and let self-compiling code use guards freely. Roadmap: `PLAN.md` native-backend section.

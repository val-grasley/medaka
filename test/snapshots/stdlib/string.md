...
--   • `toUpper`/`toLower` here are the *String* versions (full Unicode, so
--     "Straße" → "STRASSE").  For a single Char, call the kernel externs
--     `charToUpper`/`charToLower` (Char → Char, identity where Unicode would
--     expand 1→N — they can't return "SS").
...
-- codepoint directly; the Unicode-aware ones wrap the uucp-backed externs.
...
{- | Uppercase every character (full Unicode — expands 1→N, e.g. ß → SS).
...
{- | Lowercase every character (full Unicode).

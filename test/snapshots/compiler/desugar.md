...
-- `a.[lo..hi]` / `a.[lo..=hi]` (slice) → `sliceRange a lo hi incl` (Sliceable
-- interface dispatch, #670).  The `incl` flag (`..=` inclusive, `..` exclusive)
-- rides along as a Bool literal argument.  Routing through a constrained method
-- makes a non-container receiver a check-time "no impl of Sliceable" error, where
-- the old static-tag lowering read a String/Int/[] through the Array rep (an S0
-- run≠build memory fault).  Pure surface sugar: no dedicated typecheck/eval/lower
-- arm survives (they become dead — a stray post-desugar ESlice is a loud guard).
rewriteSugar (ESlice a lo hi incl _) =
  EApp (EApp (EApp (EApp (EVar "sliceRange") a) lo) hi) (ELit (LBool incl))
rewriteSugar e = e
...
(DFunDef false "rewriteSugar" ((PCon "ESlice" (PVar "a") (PVar "lo") (PVar "hi") (PVar "incl") PWild)) (EApp (EApp (EVar "EApp") (EApp (EApp (EVar "EApp") (EApp (EApp (EVar "EApp") (EApp (EApp (EVar "EApp") (EApp (EVar "EVar") (ELit (LString "sliceRange")))) (EVar "a"))) (EVar "lo"))) (EVar "hi"))) (EApp (EVar "ELit") (EApp (EVar "LBool") (EVar "incl")))))

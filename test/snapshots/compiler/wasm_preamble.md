...
floatRuntimeLines = [
  ...
  "  ;; which is round-half-to-even).  round(x) = copysign(floor(abs(x)+0.5), x); matches",
  "  ;; native mdk_round for every finite value with |x| < 2^52 (integers above are already",
  "  ;; exact and pass through: abs+0.5 rounds back).",
  "  (func $mdk_round (param $x f64) (result f64)",
  "    (f64.copysign",
  "      (f64.floor (f64.add (f64.abs (local.get $x)) (f64.const 0.5)))",
  "      (local.get $x)))",

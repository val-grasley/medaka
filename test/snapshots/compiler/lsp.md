...
-- collect the symbols.  Exported for `medaka_symbols` (#255) — parse-only, no
-- typecheck, so it's a pure function of `src` and safe for the stateless MCP
-- disk-read harness (mirrors `typeAtPoint`'s export shape).

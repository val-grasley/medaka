...
## Current status (2026-07-04) — error-message quality workstream: runtime `run`-path diagnostics + typecheck actionable-fix cluster DONE, corpus 11.15→11.66/14 (`6e981912`)
...
- **Open (error-quality follow-ons), ranked (typecheck actionable-fix reservoir now DONE — see above):** (1) **mechanical cleanup sweep:** parse-path `help`/`fix` channel (parse errors are a flat `ParseError Int Int String` with no help/fix slot — adding one retrofits machine `fix` onto the F5 syntax carveouts + `/=`→`!=`); missing-constraint/missing-case `help` lift (the typecheck help/fix channel exists now); unknown-module did-you-mean (raw string in `loader.mdk`, not a `ResError`). (2) heavier/optional: **native `build`+run-path** runtime errors (the C `mdk_panic` sites — deferred Fork D from `RUNTIME-DIAGNOSTIC-CHANNEL-DESIGN.md`; needs locations in emitted IR → forces a seed re-mint). (3) absolute-floor fixtures — `main_takes_unit` (build-only, 3/14) + the silent-accept `ambiguous_return`/`redundant_arm` (4/14 each); these are build/check-path silent-accepts (accept-what-should-error), a different + harder class than message quality. F remains the corpus's weakest axis (0.69/2) — the remaining help-only hints have a genuine F=1 ceiling (two valid alternative edits, not one). A possible cheap F 0→1 on the runtime cluster: name the list length in `E-INDEX-OOB`.

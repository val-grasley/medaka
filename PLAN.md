...
- **Next tandem bites (scouted, ranked):** DISTINCT (small — dedup on projected cells via `cellListEq`); UPDATE…SET `<expr>` (needs a Cell-producing expression evaluator — `SqlExpr` has no arithmetic node yet); subqueries (large — `SqlExpr`/`Select` recursion). Index-use is a perf rewrite with no observable tandem diff → deprioritized. (LEFT JOIN ✅ landed above.)

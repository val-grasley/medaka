...
| 🔴 eval: constrained cross-module unspecialized DEFAULT method `no matching impl for dispatch` (run≠build) | Compiler / language | 🔴 OPEN (found 2026-06-29) — `impl Foldable Bag` (user file) + `foldMap (x=>[x,x]) (Bag […])`: `medaka build` (LLVM) correct, but `medaka run` (`eval.mdk`) panics `no matching impl for dispatch`. UNconstrained default (`length`@Bag) runs fine; the CONSTRAINED default doesn't. Same class as the `fillImplDefaults` same-module-only specialization (cross-module impls keep the default; eval can't route the constrained default's dict). Inverse of the usual run=oracle pattern |

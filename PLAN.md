...
| **SQLite read-path library (capstone)** | [`SQLITE-DESIGN.md`](./SQLITE-DESIGN.md) | 🟢 **reads real `.sqlite` files (verified vs `sqlite3`)** | LANDED: foundation externs (`readFileBytes` + bitwise, `1b25c9b`, fixpoint YES); `byteparser/` binary parser-combinator lib (`986bbd4`); cross-project `[dependencies]` in the native loader (`0ad8ae9`, fixpoint YES); file-format reader (`6657238`) — header/pages/records/`sqlite_master`; multi-page B-tree traversal (`86b1ffa`) — header→leaf+interior pages→records→rows, byte-identical to `sqlite3` (single-leaf + 3000-row multi-page verified). NEXT: `RowType` typed-result marshalling + SELECT executor → `bytesToFloat64` extern + float columns → Phase-2 ADT query model. Fast-follows: WasmGC port (bytes-first API makes it additive) + async SQL server (the real `<Net>`/async forcing function). Minor dogfood findings deferred → [Known parser gaps](#known-parser-gaps-selfhost-parsermdk). |

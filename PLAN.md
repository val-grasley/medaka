...
## Current status (2026-07-03) — error-message quality workstream: Tiers 1–4 + `==`→Eq/`<`→Ord + Tier-3 framing + F-axis did-you-mean + Haskell name-alias + syntax carveouts + F3 resolver-diag locations (`44422ddc`)
...
- **Open (error-quality follow-ons):** parse-path `help`/`fix` channel (would give the syntax carveouts + `/=` machine fixes); unknown-module did-you-mean (raw string in `loader.mdk`); missing-constraint/missing-case `help` lift (the typecheck help/fix channel exists). The biggest untouched reservoir is the **runtime `run`-path diagnostics** (div0/index-oob/panic emit no location/JSON — the eval stage, ~5 fixtures stuck at A=0; needs a structured runtime-error channel).

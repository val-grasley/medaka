...
- **Two latent emitter gaps surfaced (worked around, NOT fixed — both in the self-compile graph, out of scope):** (1) `scanExprRecords` (`compiler/backend/llvm_emit.mdk`) traverses `CArray` but not `CList`, so a record constructed inside a **list literal** never registers its field layout → `CFieldAccess: unknown field` (worked around by lifting `Rule` records to named top-level bindings); one-line fix: `scanExprRecords (CList es) = scanExprsRecords es`. (2) Cross-module field access on a record with a **function-type field** (`Rule.check`/`Rule.fix`) SIGSEGVs (the field-scanner doesn't register that layout in the `medaka_cli.mdk` context) → all `Rule`-record access is kept inside `lint.mdk`; the CLI only passes/receives plain data. Both filed in the Open issues index.
...
| 🟡 emitter: `scanExprRecords` traverses `CArray` but not `CList` → a record constructed inside a list literal misses field-layout registration → `CFieldAccess: unknown field` (surfaced building `medaka lint`; worked around by lifting records to named bindings) | Compiler / language (emitter) | one-line fix `scanExprRecords (CList es) = scanExprsRecords es` in `compiler/backend/llvm_emit.mdk`; in self-compile graph |
| 🟡 emitter: cross-module field access on a record with a **function-type field** SIGSEGVs (field-scanner doesn't register that layout in the importing module's context; surfaced accessing `Rule.check`/`Rule.fix` from `medaka_cli.mdk`; worked around by keeping all such access inside the defining module) | Compiler / language (emitter) | `compiler/backend/llvm_emit.mdk` field-scanner; in self-compile graph |

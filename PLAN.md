...
- **Type display drops constraints** (found 2026-07-06). The scheme pretty-printer (`ppScheme`, `compiler/types/typecheck.mdk`) omits the leading constraint context: `add_ x y = x + y` displays as `add_ : a -> a -> a` instead of `add_ : Num a => a -> a -> a`. Affects `medaka check --types`, the LSP hover, AND the playground hover (all route through `ppScheme`/`hoverFor`). The constraint IS correctly inferred (dispatch works) — only the DISPLAY drops it, so this is display-only: `ppScheme` should render `<constraints> => ` when the scheme's constraint set is non-empty. Small (→ recapture `check --types`/hover goldens, no re-mint). Relevant to 0.1.0 (hover types are a playground/editor feature).

...
## Current status (2026-07-04) — error-message quality workstream: runtime `run`-path diagnostics DONE (located text + `--json`), corpus 11.15→11.48/14 (`b58cedff`)
...
- **Open (error-quality follow-ons), ranked:** (1) **typecheck actionable-fix reservoir** — the `typecheck/` type-mismatch cluster (`apply_non_function`, `if_branch_mismatch`, `list_heterogeneous`, `cons_type_mismatch`, `arg_order_swapped`, `wrong_arg_type_in_map`) got Tier-3 *framing* but is still **F=0** (no `help`/`fix`); the typecheck help/fix side-channel now exists, so this is a lift — the biggest remaining compile-time reservoir. (2) mechanical cleanup sweep: parse-path `help`/`fix` channel (retrofits machine `fix` onto the F5 syntax carveouts + `/=`); missing-constraint/missing-case `help` lift (channel exists); unknown-module did-you-mean (raw string in `loader.mdk`). (3) heavier/optional: **native `build`+run-path** runtime errors (the C `mdk_panic` sites — deferred Fork D; needs locations in emitted IR → forces a seed re-mint). Absolute-floor fixtures `ambiguous_return`/`redundant_arm`/`main_takes_unit` are build/check-path silent-accepts, not run-path — out of this reservoir's reach. A possible cheap F 0→1 on the runtime cluster: name the list length in `E-INDEX-OOB`.

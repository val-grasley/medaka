...
| **SQLite read-path library (capstone)** | [`SQLITE-DESIGN.md`](./SQLITE-DESIGN.md) | 🟡 **foundation landed; read-path slices next** | Foundation externs (`readFileBytes` + bitwise) landed `1b25c9b`, fixpoint YES. Design forks A–E resolved. Next: `byteparser/` project (slice 1) → header parser (2) → page/cell walker (3) → record decoder + `bytesToFloat64` extern (4) → schema read (5) → B-tree multi-page walk (6) → SELECT executor (7) → `RowType` marshalling (8) → Phase 2 query ADT (9). Fast-follows: WasmGC port (bytes-first API makes it additive) + async SQL server (the real `<Net>`/async forcing function). |

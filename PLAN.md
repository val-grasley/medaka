...
| 🔴 Constrained fn passed as a first-class HOF arg (`filter myParity [..]`) → BUILD fails `unsupported Core IR node CDict` (run OK) | Compiler / language | 🔴 OPEN (2026-06-28) — pre-existing emitter gap; emit a CDict-bearing closure for a constrained fn flowing into a HOF; memory `project_sqlite_review_batch` |

...
## Current status (2026-07-01) — tandem SQL: ORDER BY + INNER/LEFT JOIN + DISTINCT + arithmetic/UPDATE-SET-expr (`4d6d502`)

Five tandem SQL features (each works identically native + wasm, auto-gated by `test/wasm/diff_sqlite.sh` — now 8/0), all **pure library** (`sqlite/lib/{select,mutate}.mdk`) → NO seed re-mint / fixpoint concern. Each independently orchestrator-verified vs the real `sqlite3` CLI AND native==wasm, then merged. Continues the dogfood-soak thesis (real library work flushes compiler bugs — none surfaced across all five; every order/join/distinct/arithmetic shape ran `run`==`build`==native==wasm, including the type-lost-Float-prone Float-in-Cell arithmetic). Owning doc: SQLite design docs; memory: `project_sqlite_mutation_and_wasm` (tandem workflow).
...
- **Next tandem bites (scouted, ranked):** computed SELECT columns (`SELECT a+b` — the evaluator now exists; needs projection-expression plumbing since projection is the `RowType`, not a `Select` field); subqueries (large — `SqlExpr`/`Select` recursion); string functions / `||` concat (extend the evaluator to text). Index-use is a perf rewrite with no observable tandem diff → deprioritized. (arithmetic + UPDATE-SET-expr + DISTINCT + LEFT JOIN ✅ landed above.)

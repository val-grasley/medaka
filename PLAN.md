...
**Design note — explicit `show` (superseded).** Originally each hole had to be a `String`, so users wrote `\{show age}` rather than have the type checker auto-insert `show`. **Revised:** a hole `\{e}` now desugars (in `lib/desugar.ml`, via `rewrite_sugar`) to `display e`, where `Display` is a dedicated display interface (Debug-vs-`Show` split) defined in `core.mdk`. `Display String`/`Char` do *not* quote; all other instances mirror `show` but recurse with `display`. User types get an instance via `deriving (Display)` (`derive_display_data`/`derive_display_record`). The lowering flows through the marker + dict-passing like any interface method, so the old `typecheck.ml`/`eval.ml` `EStringInterp` arms are now dead on the typed path (kept as a fallback; `resolve.ml`/`printer.ml` still use the node pre-desugar). A hole whose type has no `Display` instance is a type error — no silent fallback.

...
## Current status (2026-07-01) — tandem SQL: multi-column ORDER BY + INNER JOIN (`0d599c1`)

Two tandem SQL features (each works identically native + wasm, auto-gated by `test/wasm/diff_sqlite.sh`), both **pure library** (`sqlite/lib/select.mdk`) → NO seed re-mint / fixpoint concern. Each independently orchestrator-verified vs the real `sqlite3` CLI AND native==wasm, then merged. Continues the dogfood-soak thesis (real library work flushes compiler bugs — none surfaced this round; every join/order shape ran `run`==`build`==native==wasm). Owning doc: SQLite design docs; memory: `project_sqlite_mutation_and_wasm` (tandem workflow).
...
- **Next tandem bites (scouted, ranked):** LEFT JOIN (superset of INNER — null-pad unmatched left rows + `tOption` right decode; interacts with WHERE-vs-ON); UPDATE…SET `<expr>` (needs a Cell-producing expression evaluator — `SqlExpr` has no arithmetic node yet); DISTINCT (small — dedup on projected cells via `cellListEq`); subqueries (large — `SqlExpr`/`Select` recursion). Index-use is a perf rewrite with no observable tandem diff → deprioritized.

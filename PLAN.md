...
| 🔴 Constrained user fn shadowing a prelude free fn → `run≠build` (`unbound $dict_<name>_0` on run, build OK) | Compiler / language | 🔴 OPEN (2026-06-27) — Phase-134-class joint dict-pass collision (collect_arities by bare name); BLOCKS re-adding prelude `isEven`/`isOdd`; memory `project_sqlite_review_batch` |

...
**Known-open item from this arc:** a typeclass impl defined in a non-prelude sibling module fails to emit its `define` at build time (call emitted, define missing) — reproduces with plain `Result`/`Box`, not just tuples; prelude impls are unaffected. Eval-only fixtures capture the shape (`test/eval_typed_modules_fixtures/{bimappable_tuple_sibling,bimappable_constrained_sibling,impl_requires_nonfunctor_sibling}/`). See `compiler/EMITTER-GAPS.md` "Cross-module sibling-impl-emit gap" for the pointer. **Deferred to P2** (per `FP-STDLIB-DESIGN.md`): monoid newtypes, Reader/Writer/State, zipWithThen/Kleisli/asum, Enum, lazy Seq.

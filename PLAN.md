...
| **WasmGC backend (2nd backend)** | [`selfhost/WASMGC-DESIGN.md`](./selfhost/WASMGC-DESIGN.md) §9 (slices) + [`selfhost/WASM-SELFHOST-ROADMAP.md`](./selfhost/WASM-SELFHOST-ROADMAP.md) (self-host gap-closing) | 🟢 **MVP MET + W8b DONE; self-host-on-WasmGC IN PROGRESS** (2026-06-22) | Direct Core IR→WAT emitter. Compute+print MVP (W1–W9b+W8b) byte-identical to `medaka build`. **Now driving the emitter to compile the WHOLE compiler (self-host on WasmGC → frontend-only playground).** Census-driven: a gap-record mode (`wasm_emit_gaps_main.mdk`) enumerates what the emitter can't lower over the full compiler graph; **1428 → 82 gaps closed (94%)** — panic, array intrinsics, Ref (`$refbox`), `__fallthrough__`, string-literal clause heads, charCode, destructuring/refutable/assign let-binds, UTF-8 codec externs. diff_wasm 85→115. `wasm_emit.mdk` is OUTSIDE the compiler graph → no fixpoint/seed; output-diff gate is decisive. **Remaining (~82):** nested-closure free-var capture (do/where/let-group, in progress), small structural (unknown-ctor/non-Int-switch/PVar-only-params/dict-param ~20), IO host surface (readFile/args/getEnv ~4), then in-browser WAT assembly + the self-host run. Lazy-emitter rule: binding/label state threads as data, not mutable refs. |

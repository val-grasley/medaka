...
## Current status (2026-07-01) — tandem SQL: multi-column ORDER BY + INNER/LEFT JOIN + DISTINCT (`ea68fdd`)

Four tandem SQL features (each works identically native + wasm, auto-gated by `test/wasm/diff_sqlite.sh` — now 7/0), all **pure library** (`sqlite/lib/select.mdk`) → NO seed re-mint / fixpoint concern. Each independently orchestrator-verified vs the real `sqlite3` CLI AND native==wasm, then merged. Continues the dogfood-soak thesis (real library work flushes compiler bugs — none surfaced this round; every join/order/distinct shape ran `run`==`build`==native==wasm). Owning doc: SQLite design docs; memory: `project_sqlite_mutation_and_wasm` (tandem workflow).
...
- **Next tandem bites (scouted, ranked):** UPDATE…SET `<expr>` (needs a Cell-producing expression evaluator — `SqlExpr` has no arithmetic node yet; this evaluator would also unlock computed SELECT columns); subqueries (large — `SqlExpr`/`Select` recursion). Index-use is a perf rewrite with no observable tandem diff → deprioritized. (DISTINCT + LEFT JOIN ✅ landed above.)

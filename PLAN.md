...
## Current status (2026-07-03) — error-message quality workstream: Tiers 1–4 + `==`→Eq/`<`→Ord + Tier-3 framing + F-axis did-you-mean + Haskell-alias hints + F3 resolver-diag locations (`9d6398ad`)
...
- **Open (F-axis follow-ons):** F5 Haskell SYNTAX carveouts (`::`/`:` cons-vs-sig, `\x ->`, `case…of` — parse-error tier, harder — the natural next friendliness win); unknown-module did-you-mean (the error is a raw string in `loader.mdk`); missing-constraint/missing-case `help` lift (the typecheck help/fix channel now exists — a future pass can reuse it). The biggest untouched reservoir remains the **runtime `run`-path diagnostics** (div0/index-oob/panic emit no location/JSON — the eval stage, ~5 fixtures stuck at A=0).
